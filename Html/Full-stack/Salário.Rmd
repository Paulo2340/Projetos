...
```{r}
limpo %>% filter(Contrato %in% c("CLT", "PJ"))%>% count(vars = Contrato) %>% mutate(proporcao = n / sum(n) * 100) %>% ggplot(aes(x = "", y = n, fill = vars)) + geom_col() + theme_void() + coord_polar(theta = "y", start = 0) + geom_text(aes(label = paste(proporcao, "%")))
```
